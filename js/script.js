document.addEventListener("DOMContentLoaded", () => {
  ...
  if (senhaInput) {
    senhaInput.addEventListener("input", limpaErro);
  }
  ...
  if (videoRect) {
    ...
    const finalizarArraste = (y) => {
      ...
      if (delta < -LIMIAR_ARRASTE) videoAnterior();
    };
    ...
    videoRect.addEventListener("wheel", (e) => {
      e.preventDefault();
      if (e.deltaY > 0) proxVideo();
      else videoAnterior();
    }, { passive: false });
  }
  ...
  if (btnSave) {
    btnSave.addEventListener("click", () => {
      ...
      const ja = lista.some((v) => v.src === videoAtual.src);
      ...
      salvaLista(lista);
      btnSave.classList.toggle("saved", !ja);
    });
  }
